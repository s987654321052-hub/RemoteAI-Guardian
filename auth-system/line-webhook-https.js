/**
 * RemoteAI Guardian - HTTPS Webhook 服務
 * 支援 HTTPS 和 HTTP
 */
...
class HttpsLineWebhook {
  ...
  async sendLineMessage(userId, message) {
    try {
      ...
      console.error(`❌ 發送失敗:`, error.response?.data?.message || error.message);
      return false;
    }
  }
  ...
  start(port = 3001) {
    ...
    if (fs.existsSync(keyFile) && fs.existsSync(certFile)) {
      try {
        ...
        httpsServer = https.createServer(options, this.app);
        useHttps = true;
        console.log(`\n🔐 HTTPS 伺服器已啟動 (使用自簽證書)`);
      } catch (error) {
        console.log(`⚠️ HTTPS 初始化失敗: ${error.message}`);
        console.log(`📍 將使用 HTTP 模式`);
      }
    } else {
      console.log(`⚠️ SSL 證書未找到 (${certDir})`);
      console.log(`📍 將使用 HTTP 模式`);
      console.log(`\n💡 要啟用 HTTPS，請運行:`);
    ...
    if (useHttps && httpsServer) {
      httpsServer.listen(port, '0.0.0.0', () => {
        console.log(`\n✅ RemoteAI Guardian HTTPS Webhook 已啟動`);
        console.log(`🔐 HTTPS Webhook: https://100.127.44.67:${port}/webhook/line`);
        console.log(`❤️  健康檢查: https://100.127.44.67:${port}/health\n`);

        // 自動發送初始化訊息
        if (this.lineUserId) {
          setTimeout(() => {
            this.sendLineMessage(this.lineUserId, '✅ RemoteAI Guardian HTTPS 已啟動！\n\n現在可以開始使用。試試發送 "help" 命令');
          }, 2000);
        }
      });
    } else {
      ...
      httpServer.listen(port, '0.0.0.0', () => {
        console.log(`\n⚠️ RemoteAI Guardian HTTP Webhook 已啟動 (HTTPS 不可用)`);
        console.log(`📍 HTTP Webhook: http://100.127.44.67:${port}/webhook/line`);
        console.log(`❤️  健康檢查: http://100.127.44.67:${port}/health`);
        console.log(`\n💡 LINE 要求 HTTPS，請配置 SSL 證書\n`);
      });
    }
  }
}
...
if (require.main === module) {
  const webhook = new HttpsLineWebhook();
  const port = process.env.LINE_WEBHOOK_PORT || 3001;
  webhook.start(port);
}

module.exports = HttpsLineWebhook;
